...
import net.sourceforge.vietpad.VietUtilities;
...
public class StripDiacriticsServlet extends HttpServlet {
	private static final Logger log = Logger
			.getLogger(StripDiacriticsServlet.class.getName());
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp)
			...
			try {
				...
				if (respCode == HttpURLConnection.HTTP_OK) {
					...
					while ((line = reader.readLine()) != null) {
						respBuf.append(
								fixSmartQuotes(VietUtilities
										.stripDiacritics(line.trim()))).append(
								"\n");
					}
					reader.close();
				} else {
		...
		resp.getWriter().println(respBuf.toString().trim());
	}
